...
def minimax_with_stats(start: Tuple[int, ...], goal: Tuple[int, ...], max_depth: int = 4) -> Tuple[Optional[List[Tuple[int, ...]]], int]:
    visited_count = 0

    def minimax(state: Tuple[int, ...], depth: int, maximizingPlayer: bool, prev_state: Optional[Tuple[int, ...]] = None) -> Tuple[int, List[Tuple[int, ...]]]:
        ...
        if maximizingPlayer:
            ...
            for child in valid_children:
                value, path = minimax(child, depth - 1, False, state)
                # Dùng >= thay vì > để khi các bước có heuristic bằng nhau, 
                # nó vẫn ưu tiên tiến lên thay vì bị kẹt
                if value > bestValue:
                    bestValue = value
                    bestPath = [child] + path
            ...
            bestPath = []
            for child in valid_children:
                value, path = minimax(child, depth - 1, True, state)
                if value < bestValue:
                    bestValue = value
                    bestPath = [child] + path
            return bestValue, bestPath
    ...
    # Mô phỏng tối đa 50 bước để tránh vòng lặp vô tận (do MIN luôn phá)
    for _ in range(50):
        if current == goal:
            break
            
        val, best_path = minimax(current, max_depth, maximizing, prev)
        if not best_path:
            break
